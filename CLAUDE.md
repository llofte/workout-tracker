...
## ⚠️ SwipeBack regression — v195 changes tack entirely, still awaiting on-device confirmation

**Symptom reported by user:** after tapping into a session from Home's Recent list, starting a left-edge swipe-back, stopping mid-drag, lifting that finger, then touching the page again — the whole screen becomes freely draggable in any direction with the Home screen visible behind it. Same failure mode as [[reference_ios_swipe_gesture_conflict]].

**v193 (WRONG — didn't fix it):** guarded against a second *simultaneous* finger resetting gesture state mid-drag. User confirmed the real repro is sequential (lift, then re-touch), not simultaneous multi-touch, so this guard was solving a bug that wasn't the reported one.

**v194 (ALSO WRONG — didn't fix it):** theorized `start()`'s unconditional `setAnimating(false)` on every touchstart was cancelling an in-flight 220ms snap-back CSS transition mid-flight when a non-edge tap landed during it. Fixed and verified at the JS-state level (confirmed via synthetic `TouchEvent`s that a stray tap no longer disturbs the transition) — **but user confirmed this still didn't fix the actual on-device bug.** Both v193 and v194 only ever edited `SwipeBack.jsx`, and that reasoning turned out to be off-track.

**v195 — different tack, based on a structural finding rather than SwipeBack.jsx logic:** asked the user three clarifying questions and learned the bug **only happens on session detail**, never on Movement Detail or the All-Workouts list — even though both of those *also* use the identical `SwipeBack.jsx` component. That ruled out `SwipeBack.jsx`'s internal logic as the culprit entirely (its code is shared and identical across all three screens). Investigated how each screen mounts `SwipeBack` and found the actual structural difference: `MovementDetailScreen` mounts as `<SwipeBack><MovementDetailScreen /></SwipeBack>` directly, in-tree, no portal. `SessionDetailScreen` (opened from Home's Recent list, `HomeScreen.jsx` — the exact path in the user's repro) is rendered via `createPortal(..., document.body)`, wrapped in a `position: fixed` div, which itself wrapped a *second*, separate `overflowY: 'auto'` scrollable div, before `SwipeBack`. This is the only screen with a portal + doubled-wrapper structure. A `useLayoutEffect` right above it restores `main.scrollTop` on close, confirming the portal is deliberate (isolating this overlay from Home's own scroll container) — not something to rip out blindly. Fix: collapsed the two nested wrapper divs into one (still fixed-positioned, still portaled, still scrollable) rather than removing the portal. Verified in preview: tab bar still visible, session detail still renders and scrolls correctly, and a full swipe-back gesture still completes and calls `onBack` via simulated `TouchEvent`s.

**v195 STILL didn't fix it** — user reports it changed behavior though: previously the page would snap back into place on finger-lift; after v195 it now stays floating/frozen on lift instead. That's a real, useful signal (v195's wrapper-div collapse changed *something* about event delivery) but not enough to keep guessing blind — this is the 3rd consecutive wrong guess in a row with zero on-device visibility.

**v196 — stopped guessing, added on-screen instrumentation instead:** per the [[reference_ios_pwa_chin_gap]] lesson ("diagnose with an on-screen readout, not guesses"), `SwipeBack.jsx` now has a `DebugOverlay` — a small black bar pinned to the top of the screen (`zIndex: 99999`, `pointerEvents: 'none'` so it doesn't block touches) that logs every `touchstart`/`touchmove`(direction-lock only)/`touchend` event with the relevant state (`active`, `dx`, `locked`, `touches.length`, which branch of `end()` ran). **This is diagnostic-only and must be removed once the real bug is found** — it's clearly marked `TEMPORARY DEBUG` in the code. Ask the user to reproduce the exact repro on their phone and screenshot or read back what the black bar shows — that tells us definitively whether `touchend` is firing, what `active`/`dx` are at that moment, and which `end()` branch (full-close / snap-back / no-op / ignored) executes, instead of reasoning about it blind.
