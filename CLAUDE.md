...
**Two wrong guesses, one structural fix that helped but didn't close it, then the actual mechanism:**
- **v193 (wrong):** guarded against a second *simultaneous* finger. User confirmed the repro is sequential (lift, then re-touch) — not simultaneous, so this solved a bug that wasn't the real one.
- **v194 (wrong):** theorized a non-edge tap was cancelling the in-flight 220ms snap-back CSS transition via `setAnimating(false)`. Verified the JS-state mechanism directly, but user confirmed it still didn't fix the on-device bug.
- **v195 (real structural issue, but not the root cause):** found that `SessionDetailScreen` (only when opened from Home's Recent list) is the *only* screen rendered via `createPortal(..., document.body)` wrapped in a doubled set of nested divs — `MovementDetailScreen`/`AllWorkouts` mount `SwipeBack` directly with no portal, and don't have this bug. Collapsed the redundant nesting. Still didn't fix it, but user reported the failure mode itself changed (page now froze/stayed floating on lift, instead of needing a second touch first) — a sign v195 changed something about event delivery/timing, even though it wasn't the fix.
- **v196 — stopped guessing, added instrumentation:** per [[reference_ios_pwa_chin_gap]]'s "diagnose with an on-screen readout, not guesses" lesson, added a visible on-screen debug log to `SwipeBack.jsx` that called `setDebugLog(...)` (forcing a React re-render) on every `touchstart`/`touchend`, **including ones that would otherwise hit an early `return` and do nothing at all.**
