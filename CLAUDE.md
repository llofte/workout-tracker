...
**Current version:** v228 / `bb-wod-v203` (`HomeScreen.jsx` badge / `public/sw.js` CACHE). Not yet committed or pushed — see below.

**v228 — Fixed PR badge over-marking + added a "MAX" state for tied (not beaten) records.** User complaint: PR badges were showing on sets that only matched an existing record, not just the set that actually achieved it first. Root cause: `computeSetPRStatus()` (`SessionDetailScreen.jsx`) recomputed PR status at display time using `totalLoad >= best.weight` (greater-than-**or-equal**) — so every set tying the historical best got flagged 'current' (PR badge), not just the one that actually set it. The save-time flag (`set.isPR`, written once by `detectPRs()` in LogScreen.jsx with a strict `>` comparison) already correctly identified "first time achieving" both within a workout and across workouts — the display layer just wasn't trusting it.
- `computeSetPRStatus()` now trusts `set.isPR` as authoritative: `'current'` only when `isPR` is true and it's still the best on file; `'former'` when `isPR` is true but a later session has since beaten it (unchanged from before).
- New `'max'` state: a set that exactly ties the current best but didn't set it, **and** the record it's tying belongs to a *different* session — i.e. matched (not beaten) in a later workout. A same-session tie (e.g. a second identical set right after the one that just set the record) still gets no badge at all, matching "only note it the first time."
- Required threading the current session's own id down through `StrengthBlock` → `SetRows` / `MultiSetStrengthTable` → `computeSetPRStatus`, since distinguishing same-session vs. cross-session ties needs it (compared against the `sessionId` already stored on each PR record).
- New visual: `PRBadgeLabel` now takes a `text` prop (defaults to `"PR"`) so the same bolt-icon badge component renders `"MAX"` in a distinct amber (`#d4af37`) instead of the teal (`#0ff7c5`) PR color — same treatment applied to both `SetRows` (single-movement view) and `MultiSetStrengthTable` (multi-movement synced table, including the row-level label column).

**Verified live end-to-end** with two temporary test sessions (same movement, same weight/reps, logged separately since PR detection intentionally skips a movement's very first-ever log per an existing documented rule): Session B (which actually set the record) correctly showed "PR" on set 1 only, no badge at all on its identical set 2. Session A (a separate workout matching that same weight without ever setting it) correctly showed "MAX" on both its matching sets. Cleaned up both test sessions and the test movement record afterward. **Known pre-existing gap, not touched**: `MultiSetStrengthTable` still doesn't render anything for the `'former'` status (a set that once held a record, since beaten) — `SetRows` already shows a small dim "PR" label for this case, but the multi-table never checked for it even before this fix. Flagging in case it comes up later.

**v227 — Bodyweight strength sets (weight 0) no longer show "0 lbs".** User request: just show reps. Fixed in `SessionDetailScreen.jsx`: `SetRows`' `weightText` (single-movement view) now returns empty string specifically for `set.weight === 0` (kept `null` → "—" unchanged, for a genuinely-unset weight); `MultiSetCell` (multi-movement synced table) now skips rendering the "· N lbs" segment entirely when `weight === 0`, showing just "xN". Scoped to strength sets only, per the request — metcon movement weight display untouched. Verified live with a temporary bodyweight test movement (Air Squat, 15 reps, weight 0) — confirmed it displayed as "15 reps" with no weight text in both the page and the card. Deleted the test session and its resulting test PR record afterward.

**v226 — Photo-parse prompt fix: "C&J" misread as "squat thrusters" on a real board.** User provided the actual whiteboard photo that caused this (Monday: Strength Speed Back Squat, Metcon "10 Rounds / 5 C&J / 10 cal Bike"). Investigated the handwriting first — "C&J" is written plainly, not visually ambiguous, ruling out an OCR/legibility explanation. Found the real cause in `buildPhotoPrompt()` (`LogScreen.jsx`): the abbreviations list was split into two separate, non-overlapping lists — one under "Strength rules" (which had `C&J=Clean & Jerk`) and one under "Metcon rules" (which didn't). On this board, "C&J" appears in the *Metcon* section, so the mapping that would have caught it was scoped to the wrong section entirely. Also found no instruction anywhere telling the model to transcribe literally rather than substitute a plausible-looking common movement — meaningful because Thrusters + cal Bike is an extremely standard partner-WOD pairing, giving the model a specific wrong-but-plausible thing to default to.
- Merged both abbreviation lists into one, moved to a section-agnostic position (right after "Section classification", before "Strength rules"), explicitly stated to apply in any section — so an abbreviation known in one context is always known in every context, and this class of gap can't silently reopen by two lists drifting apart again.
- Added an explicit anti-substitution instruction at the very top of the prompt: transcribe movement names literally, never swap in a different/more common/contextually-expected movement, even under illegible handwriting or an unusual pairing — give a best literal reading instead of guessing a well-known alternative.

**Not verified against a real Claude photo-parse call** (would cost real API credits) — this is a prompt-engineering fix, not testable through this text-based session the way code changes are. Confirmed the file still compiles clean and the merged prompt reads coherently. **If "C&J" (or another abbreviation) gets misread again after this, check whether the raw JSON Claude returned actually contains the wrong movement name (a genuine model misread) vs. whether the display/normalization logic mangled a correct raw name — this fix only addresses the former.**
...
2. **Set numbers went stale/duplicated after any add or delete** (e.g. "2, 2, 3, 4, 5, 6, 7"). Root cause: every add function (`addWorkingSet`, `addWarmupSet`, `addMultiRound`, `addMultiWarmupRound`) derived the new set's `num` from the current array *length*, and every delete function (`deleteSet`, `removeMultiRound`) never renumbered the remaining sets at all — so a delete followed by an add reused an already-taken number. Multi mode never visibly showed this (it labels rounds by loop index, not `set.num`), so the corruption stayed invisible until switching to Single mode, which displays `num` directly — exactly matching the report. Fixed with a new shared `renumberSets()` helper (re-derives every set's `num` from its position, independently for warmup "W1, W2…" and working "1, 2, 3…") applied after every add/delete in both modes, and now also on every `handleStrengthModeChange` direction (previously the code comment literally said "Switching Multi -> Single is always safe as-is" — it wasn't).
3. **New sets ("+ Set"/"+ Warmup") started blank instead of carrying forward the last set's numbers.** `addWorkingSet`/`addWarmupSet`/`addMultiRound`/`addMultiWarmupRound` now seed the new set's `reps`/`weight` from the last existing set of the same type (per movement, independently, in Multi mode) instead of starting empty.

**Verified live** end-to-end with a temporary test movement: confirmed a delete-then-add no longer produces duplicate numbers (correctly showed "1, 2" not "1, 2, 2"), confirmed the new set auto-filled 5 reps/100 lbs from the prior set, and confirmed adding a 2nd movement (auto-switches to Multi + OTM selected) then removing it back to 1 correctly snapped both Traditional and Single back to active. Discarded the test session afterward (never saved).

**v223 — Stopped implement/modifier text from getting baked into canonical movement names.** User complaint: things like "Alt DB Step Up" and "Barbell Row" were being logged as their own new movement names instead of normalizing to the base movement ("Step Up", "Row") with implement/variation captured separately — fragmenting PR history and movement lookup across near-duplicate names. Root-caused in `movements.js`'s `normalizeMovement()`: the old fallback (`IMPLEMENT_PREFIXES`) only stripped a leading 2-letter abbreviation ("BB "/"DB "/"KB "/"MB ") anchored at the very start of the string — so a leading modifier word ("Alt DB...") or a full-word implement ("Barbell Row") skipped it entirely and passed through untouched. Went through the user's decisions one at a time on several judgment calls before touching code (SDHP, Single-Leg Deadlift kept as standalone exceptions like KB Swing since they're genuinely distinct movements; Split-Stance Press and Strict Press split into base name + modifier; Russian KB Swing → KB Swing + modifier Russian; MB Oblique Toss → Oblique Toss + implement Med Ball; "Sumo" never treated as a modifier, always a distinct movement).
- **`normalizeMovement()`** — replaced the narrow prefix-stripper with a generic loop (`IMPLEMENT_WORDS`, `MODIFIER_WORDS`, `SA_WORD_RE`) that recognizes implement words (abbreviation AND full word: "DB"/"Dumbbell", plus Plate which was missing entirely before) and modifier words (Alt, Strict, Weighted, Deficit, Banded, Split-Stance, Prisoner, Kneeling) in any order, any position, repeatedly — not just one hand-enumerated raw string per combination. Idempotent by construction (safe to re-run on its own already-canonical output, since `toWorkoutDisplay` calls it again on every render).
- **Guardrail found while building this**: the new generic fallback would have wrongly split "Plate Jump" (a real, live movement from the 8/5 session — jumping onto a stacked plate, where "Plate" is integral to the movement like Box Jump, not a substitutable lifted implement) into "Jump" + implement Plate. Added an explicit `ALIAS_MAP` protection entry so this class of compound name stays intact. Flag any other "object you jump onto/over" style names if the fix misbehaves on one.
- **`modifier` was previously a dead field** — `normalizeMoveIdentity()` (LogScreen.jsx) read it only to check `=== 'SA'` (to set the `singleArm` boolean) and then discarded it; it was never saved to Supabase or shown anywhere for current data, even for already-existing cases like "Deficit Deadlift". Fixed end-to-end: `normalizeMoveIdentity()` now also returns `modifier` (and honors a new `singleArm` boolean the ALIAS_MAP can set directly); `newStrengthMove()`/`newMetconMove()` initialize it; `restoreStrengthMove()`/`restoreMetconMove()` hydrate it; `buildSessionData()` persists it for strength, metcon, and accessory movements; `toWorkoutDisplay()` now prefers the persisted `move.modifier`/`move.singleArm` fields over re-deriving from the (already-canonicalized, so no longer informative) stored name, falling back to the old re-derivation only for genuinely old data.
- **New `ModifierSelector` pill row** (Alt, Strict, Deficit, Weighted, Banded, Split-Stance, Prisoner, Kneeling, Russian) added next to `ImplementSelector` in all 4 movement-row call sites (Multi-mode strength, Single-mode strength, Metcon segment, Accessory segment) — single-select, so the user can tag a variation by hand instead of typing it into the name.
- **ALIAS_MAP cleanup**: `Russian KB Swing` → `{ name: 'KB Swing', modifier: 'Russian' }`; `MB Oblique Toss` → `{ name: 'Oblique Toss', implement: 'Med Ball' }`; `Split-Stance Press` (10 variants) and `Strict Press` (`SA HK KB PRESS`/`SA HK SH.P.`) → `{ name: 'Press', implement, singleArm: true, modifier: ... }`. Also migrated 14 other pre-existing `modifier: 'SA'` entries (Overhead Lunge, DB Snatch, Devil Press, Hang Clean & Jerk variants) to the new `singleArm: true` structured field for consistency — no behavior change, just aligns them with the documented convention. **Kept as exceptions, unchanged**: `KB Swing` (original), `Sumo Deadlift High Pull`/SDHP, `Single-Leg Deadlift` — all genuinely distinct movements, not variations of a shared base.

**Verified live end-to-end**: dynamic-imported `movements.js` and confirmed `normalizeMovement()` output for 14 test strings including both originally-reported bugs, all exception cases, idempotency, and un-enumerated generic combinations ("Strict Press", "Single Arm KB Press" — neither individually aliased, both split correctly via the generic fallback). Also built a real temporary test session through the live Log form (name "Step Up", DB implement, Alt modifier pill) — confirmed the saved Supabase row stores the clean canonical `name: "Step Up"` with `implement`/`modifier` as separate fields, and the session detail page correctly reconstructs the display as "Alt DB Step Up". Deleted the test session afterward. **Old sessions logged before this fix are unaffected** — e.g. the real 8/5 session's stored movement name is still literally "Alt DB Step Up" (not retroactively rewritten); this fix only changes normalization going forward.

**v222 — Added an explicit 1 DB / 2 DB count selector, replacing a hardcoded "assume 2" behavior.** User report: several real workouts logged 1 dumbbell but the app gave no way to say so, only the ambiguous "DB" implement tag (worked around before via typing "1 DB" into the free-text notes field, e.g. the real 8/5 "Alt DB Step Up" session). Investigated and found a half-built mechanism already sitting in the code: a `dumbbellCount` field read by `detectPRs()` (LogScreen.jsx) for PR total-load math, and an orphaned `DbToggle` component — both dead, never wired to any UI or persisted by `buildSessionData()`. Finished wiring it end-to-end instead of building parallel new plumbing:
- `ImplementSelector` (`LogScreen.jsx`) — replaced the old static, non-interactive "×2" hint text (shown whenever DB/KB wasn't marked single-arm, implying "you must be using 2") with real "1 DB"/"2 DB" pills, shown only for `implement === 'DB' && !singleArm` (single-arm already means one dumbbell in one hand, so count doesn't apply there). Deleted the now-fully-folded-in `DbToggle` component. All 4 call sites (Multi-mode strength table, Single-mode strength row, Metcon segment row, Accessory segment row) now pass `dumbbellCount` through and persist it on change.
- `movements.js` — `toWorkoutDisplay()` now shows the count in the implement prefix when set (`"2 DB Front Squat"`, `"1 DB Step Up"`), falling back to bare `"DB"` when unset (old data, or user hasn't specified) — unambiguous going forward without breaking anything already logged.
- Persistence — `newStrengthMove()`/`newMetconMove()` initialize `dumbbellCount: null`; `restoreStrengthMove()` (already did) and `restoreMetconMove()` (added) hydrate it when editing; `buildSessionData()` now writes it for strength, metcon, and accessory movements (previously only strength's `detectPRs` read path existed — the save path never actually wrote it).
- Volume/PR math — `weight` is treated as *per-dumbbell* (matching the existing carry-movement convention), `dumbbellCount` as the multiplier for total load. Updated `calcStrengthVol`, `calcMetconVol`'s `addLoad`, `calcPartialRoundVol`, and `carryVol` (now prefers explicit `dumbbellCount` over the old singleArm-inferred bilateral guess) in `SessionDetailScreen.jsx`, and mirrored all of it in `HomeScreen.jsx`'s duplicate `sessionVolume()`/`carryVol`/`calcPartialRoundVol`. Also fixed `computeSetPRStatus` (was comparing raw per-dumbbell weight against total-load PR history — an inconsistency that predates this feature but would've been wrong for any 2-DB movement) to multiply by `dumbbellCount` before comparing. **This only affects movements where `dumbbellCount` is explicitly set** — every existing session has `dumbbellCount: null`, so historical volume/PR totals are completely unchanged; this is additive, not retroactive.

**Verified live end-to-end** with a temporary test session: set Front Squat to DB + "2 DB", weight 25 × 5 reps → title correctly showed "2 DB Front Squat", set row showed "25 lbs" (per-dumbbell, unambiguous next to the now-labeled name), volume correctly computed as 250 lbs (5 × 25 × 2), and it correctly flagged as a PR (Front Squat had no prior 5-rep record). Deleted the test session and its resulting test PR record afterward to leave real data untouched.

**v221 — OTM rows sharing one minute now group under a single "Min X" label instead of repeating it.** Real bug on the real 8/5 session: an EMOM had two movements (Hang Power Clean + Shoulder to Overhead) both done together within minute 1 as a superset — previously each got its own `MetconMoveRow`, so "Min 1" printed twice as two separate rows. Fixed in `SessionDetailScreen.jsx`:
- New `groupMovesByMinute(movements)` groups consecutive movements sharing the same non-null `minuteAssignment` into one array (rest moves and movements with no minute assignment always stay their own group of 1, so ungrouped/legacy data renders exactly as before).
- New `MetconMoveGroupRow({ moves, isOTM, isLadder })`: renders a single group unchanged via the existing `MetconMoveRow` when it has only one movement; for 2+, shows the "Min X"/"Min X-Y" label once on the left and stacks the grouped movements' name/reps/weight rows to its right. The shared name/reps/weight rendering was extracted out of `MetconMoveRow` into `MetconMoveContent` so both single and grouped rows use identical rendering, not a duplicated copy.
- Both `MetconBlock` and `AccessoryBlock`'s segment-rendering now call `groupMovesByMinute(seg.movements).map(...)` instead of mapping `seg.movements` directly to one row each.

**Verified live** against the real 8/5 session — "Min 1" now shows once with both Hang Power Clean and Shoulder to Overhead listed under it, Min 2-5 unaffected. Re-checked 7/27 (E2MOM, grouped-OTM with no `minuteAssignment` on any movement) for regressions — renders identically to before, no label shown, one row per movement.

**v220 — Fixed keyboard "blinking" + field scrolling out of view, both regressions from the v217 keyboard fix.** Root cause: `App.jsx`'s `visualViewport` `resize` listener (added in v217 to keep focused fields visible above the keyboard) had no debounce, so it re-fired a fresh smooth `scrollIntoView` on every micro-resize — and iOS's own QuickType/predictive-text bar changes height *while typing* (not just on keyboard open/close), so this was re-triggering on nearly every keystroke, not just on focus. Overlapping smooth-scroll animations interrupting each other produced the visible "blink"; each one recalculating against a still-settling layout produced the drift that could scroll a field off-screen. There was also a second, uncoordinated system fighting over the same events: `LibrarySheet` (LogScreen.jsx) has its own undebounced `visualViewport` listener that repositions itself independently.
- `App.jsx`: added `fieldNeedsScroll(el)` — checks the field's `getBoundingClientRect()` against the current `visualViewport` bounds and skips scrolling entirely if the field is already visible, so a settled field is left alone regardless of how many resize events fire. The `resize` listener is now debounced 150ms (only acts once the viewport has actually settled, not on every intermediate animation frame), and its correction uses `behavior: 'auto'` (instant) instead of `'smooth'`, so any residual nudge isn't visible as a jump — smooth scrolling is kept only for the initial focus-triggered scroll. Fields inside a `[data-kb-self-managed]` ancestor are skipped entirely so App's global handler doesn't fight a component that already manages its own keyboard-relative position.
- `LogScreen.jsx`: `LibrarySheet`'s root sheet `<div>` now has `data-kb-self-managed` (so App.jsx's listener leaves its search field alone), and its own `visualViewport` listener is now debounced 150ms too (was firing unthrottled on every resize/scroll event, including QuickType churn while typing in the search field).
- **Not yet verified on a real device** — same as the original v217 fix, this is real-keyboard-only behavior the desktop preview can't reproduce (no visualViewport churn without an actual on-screen keyboard). The iOS Simulator tool was attempted again and is still crash-looping in this environment ("restarting after a crash" on every `attach` call) — could not get a device screenshot. Verified only that the code compiles clean, the Log form still opens and fields still focus without console errors, and that manually dispatching 5 rapid `visualViewport` `resize` events at the (mocked) desktop viewport throws no errors. **If the blink or overscroll persists in real use, the next thing to check is whether 150ms is long enough to fully absorb QuickType bar churn on the user's actual device** — if not, try lengthening the debounce (e.g. 250–300ms) before suspecting the visibility-gate logic itself.
