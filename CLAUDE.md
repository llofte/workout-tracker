...
## ⚠️ SwipeBack "freely draggable after pause" — v199 restores the known-working overlay verbatim (opacity:0 only), awaiting on-device confirmation
...
**v199 — stopped trying to isolate the minimal mechanism, restored the exact known-working structure instead:** brought back `DebugOverlay` byte-for-byte identical to the v196 version that was confirmed fixed (same real fixed-position/full-width/padded/scrollable dimensions, same `logDebug` calls at the same call sites, same text content updates) — the **only** change is adding `opacity: 0`. `opacity: 0` (unlike `width:0/height:0`) still causes the browser to paint/composite the element normally, just with a fully transparent result — this is the closest possible approximation to "the thing that worked, but invisible." Verified in preview: no visible bar, full swipe-back still completes and calls `onBack`.

**Status: UNCONFIRMED on-device.** If v199 still doesn't fix it, the mechanism is not about paint/dimensions/mutation type at all — next step should be re-enabling the fully visible v196 `DebugOverlay` (remove `opacity: 0`) as a known-good fallback while pursuing Safari Remote Debugging for real diagnostics, rather than continuing to guess at which CSS property of the overlay matters.
