...
## Multi-Set Strength Table — column alignment RESOLVED (v191 / bb-wod-v166)

**Three separate bugs, fixed in three passes — all were real, don't reintroduce any of them:**
...
2. **Columns themselves were unequal width (fixed in v190):** the per-column wrapper div in `renderRow` (`SessionDetailScreen.jsx:488`) is `flex: 1`, which should divide the row equally — but a flex item's *automatic* minimum width is based on its content's min-content size, and `MultiSetCell`'s fixed `width: 90` child (present only on columns with a weight) forced that column's min-content floor above its fair share, stealing space from columns without a weight (e.g. bodyweight-only movements). Measured on the Jun 16 3-column session: columns came out **92px / 78px / 92px** instead of an even ~87px each. Fixed by adding `minWidth: 0` to that column div, letting `flex: 1` actually divide evenly.
3. **Visible text still off-center after 1 & 2 (the actual remaining user complaint, fixed in v191):** `MultiSetCell`'s reps span had a fixed `width: 20` + `textAlign: 'right'` (to keep the "·" dot at a consistent x-position across rows regardless of rep digit count), while the weight span was `auto`-width and flush left. That put empty invisible padding on the *left* side only (before the reps text) and none on the right (after the weight text) — so the visible glyph ink was shifted right of the box's geometric center even though the box itself measured as centered. **This is why passes 1 & 2 measured as "fixed" but the user still saw it shifted right** — `getBoundingClientRect()` on a fixed-width span returns the allocated box, not the actual ink extent; the two only diverge when alignment isn't flush. Verified the discrepancy using `document.createRange().selectNodeContents(span).getBoundingClientRect()` (true glyph bounding box) vs the span's own rect — confirmed the reps span had ~7px of invisible left-padding, weight span had 0. Fixed by removing the fixed reps width and `textAlign` trick entirely (`MultiSetCell` now just flows `reps · weight` naturally and lets the outer `justifyContent: 'center'` center the whole visible string) — this drops the dot-column-alignment behavior across rows with different rep digit counts, which was never actually validated as visually needed and directly conflicted with true centering. Verified ink-center-vs-column-center within ~1px on every row of both test sessions using the `createRange` method.

**Lesson for next time:** `getBoundingClientRect()` on an element with an explicit fixed width tells you the *box*, not the *visible ink* — if any child inside has asymmetric alignment (e.g. right-align in a wider-than-content box), the box can measure as perfectly centered while the actual rendered text is not. When verifying text-centering claims, measure with `document.createRange().selectNodeContents(el).getBoundingClientRect()` on the actual text-bearing leaf elements, not `el.getBoundingClientRect()` on a styled wrapper.
...
**Gotcha when measuring this table:** `document.querySelectorAll` text matches duplicate easily (e.g. "RDL" appears both in the Home screen's background movement chips AND in the detail overlay's table) — scope queries to the detail overlay container first, e.g. by finding the legend text and calling `.closest()`.
