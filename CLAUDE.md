...
**Cannot verify the actual iOS symptom** (system gesture claiming, visible freely-draggable screen) from the desktop Chrome preview — this whole bug class fundamentally requires on-device testing. **If v195 still doesn't fix it:** stop trying more code-level guesses in `SwipeBack.jsx` or the wrapper divs — the next step should be Safari Remote Debugging (Mac + cable, Safari → Develop menu → select the phone) to get real DOM/console output during the actual repro, since three attempts based on reasoning-without-device-access have now been spent on this.
